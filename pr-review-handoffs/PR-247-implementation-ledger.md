...
| 33 | Sonar `java:S2095` | IMPACT | `WorkerSessionServer.start` is CRITICAL because 36 integration-test callers exercise the lifecycle; GitNexus maps no production execution process to the method. The public lifecycle contract and server wiring remain unchanged. |
| 33 | Sonar `java:S2095` | RED/GREEN | SonarCloud supplied the static RED: the startup executor could escape when Netty construction threw an `Error`, because cleanup caught only `IOException` and `RuntimeException`. A transfer flag plus `finally` now closes the executor for every unsuccessful startup while preserving ownership after a successful start. An artificial `Error` injection seam was not added solely for testing. |
| 33 | Sonar `java:S2095` | VERIFY/PUBLISH | Focused lifecycle tests and Java 25 `./mvnw verify` are GREEN: 1,715 unit tests and 505 integration tests passed with zero failures or errors. The signed executor-only commit `57bfb1fb` was pushed before the code-smell tranche. |
| 34 | Sonar 30 code smells | IMPACT | `WorkerSessionGrpcService` is LOW with one direct dependent and no mapped execution flow. Indexed test classes are LOW and isolated; several new test-only classes are not yet in the graph, so their impact is recorded as unknown rather than inferred safe. |
| 34 | Sonar 30 code smells | STRUCTURAL | Documented 23 intentionally empty gRPC callbacks at the empty method body, extracted one media-source fixture from an exception assertion lambda, renamed two locals that hid the appender field, and removed four unused worker-id constants. No assertion, API, or runtime decision changed. |
| 34 | Sonar 30 code smells | VERIFY | The focused unit batch passed 24 tests. Java 25 `./mvnw verify` is GREEN in 3:40: 1,715 unit tests and 505 integration tests passed with zero failures or errors; Checkstyle, Spotless, packaging, and merged JaCoCo reporting completed successfully. SonarCloud reanalysis remains the external confirmation gate after push. |
