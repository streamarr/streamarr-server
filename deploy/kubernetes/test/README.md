...
authorization checks. Supplying `WORKER_IMAGE` adds the standalone Spring worker and real media
execution. The image must already be present in the local Docker daemon. This test never publishes
an image.
...
To include a locally built worker image, add `WORKER_IMAGE=your-worker-image:tested` to the
same invocation. The harness deploys one worker using the shipped Deployment's normal command,
...
the exact policies taken from the deployment manifest, and the server and proxy logs. Image runs
also retain the local image identity, worker logs, uploaded segment, and decode results. Cleanup
