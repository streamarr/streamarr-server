...
StreamingOptions (client)  +  MediaProbe (ffprobe)
            ...
            → TranscodeMode, ContainerFormat, needsKeyframeAlignment
                    |
            PlaybackTranscodeJobService → complete-ladder lifecycle + fencing
                    |
            TranscodeWorkerPort
                    |
            LocalTranscodeWorkerAdapter (default)
                    |
            LocalTranscodeEngine
            ├── FfmpegCommandBuilder → -map, codecs, keyframes, HLS args
            └── encoder selection + FFmpeg process lifecycle
                    |
