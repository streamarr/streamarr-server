...
Identity, sessions, and token security follow ADR 0015/0016: HMAC-signed access tokens with
nested scopes, digest-only rotating refresh tokens, and per-request version-counter checks that
fail closed. The browser client is same-origin by construction -- the Vite dev server proxies
`/graphql` and `/api` in development, and production serves the client from the same origin --
so no CORS configuration exists and none should be added.
...
the whole cache on every reconnect so missed notifications cannot leave stale entries.
...
application traffic goes through a transaction-mode pooler.
