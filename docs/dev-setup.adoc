...
[source,bash]
----
./mvnw -pl streamarr-server spring-boot:run
----
...
----
docker compose up -d postgres
./mvnw -pl streamarr-server generate-sources -Pgenerate-jooq-code
----
...
./mvnw -pl streamarr-server spring-boot:run -Dspring-boot.run.profiles=dev,observability
----
