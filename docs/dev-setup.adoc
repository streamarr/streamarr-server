...
[source,bash]
----
./mvnw -pl :streamarr-server -am spring-boot:run
----
...
----
docker compose up -d postgres
./mvnw -pl :streamarr-server -am package -DskipTests -Pgenerate-jooq-code
----
...
./mvnw -pl :streamarr-server -am spring-boot:run -Dspring-boot.run.profiles=dev,observability
----
