...
| MPEGTS
| `.ts`
| Transport stream, common in broadcast recordings. Streamarr reads it as a source container but never delivers HLS in it

| fMP4
| `.m4s` + `.mp4` init
| Fragmented MP4. Streamarr delivers every HLS variant in it, whatever the codec. Needs an initialization segment (`init.mp4`) containing codec parameters
|===
...
| Royalty-free, Netflix uses for 30%+ of traffic in 2026
|===
...
Streamarr serves every variant as multiplexed fragmented MP4 in an HLS version 6 playlist, whatever the codec (https://github.com/streamarr/streamarr-adr/blob/main/adr/0037-worker-delivers-fmp4-segments-from-ffmpeg-stdout.adoc[ADR 0037]).
Each media playlist names the variant's initialization segment (`init.mp4`) with an `EXT-X-MAP` tag, and the player loads it before any media segment.
HLS delivery is always fragmented MP4; the transcode decision does not choose it.
...
Each quality variant runs as a separate FFmpeg process on a worker.
FFmpeg writes fragmented MP4 to its standard output, and the worker groups the fragments into media segments and uploads the variant's initialization segment and each media segment to the server (https://github.com/streamarr/streamarr-adr/blob/main/adr/0037-worker-delivers-fmp4-segments-from-ffmpeg-stdout.adoc[ADR 0037]).
The server stores them under the variant's directory:
...
The server generates every playlist itself (https://github.com/streamarr/streamarr-adr/blob/main/adr/0008-server-generated-hls-playlists.adoc[ADR 0008]), so the store holds no playlist files.
One process per variant keeps each process simple (single output) and allows independent lifecycle management -- a variant can be started, stopped, or replaced by a replacement attempt without affecting others.
