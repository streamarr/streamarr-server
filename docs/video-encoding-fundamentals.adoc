...
Tiers are filtered by the source resolution (no upscaling) and any client-declared constraints (`maxWidth`, `maxHeight`, `maxBitrate`). A 720p source produces at most 720p, 480p, and 360p variants.

An explicit quality selects one rendition at that target resolution instead of an adaptive ladder. The same width, height, and bitrate constraints determine both the FFmpeg request and the master-playlist metadata; sources are never upscaled. Explicit UHD uses a 15 Mbps target when the source can supply 2160p.
