...
At the time of this decision, `TranscodeExecutor` was a process-shaped local interface: it started one rendition at a time, carried host-local paths, exposed process health, and relied on caller-managed ordering.
...
The implementation uses an application-owned `TranscodeWorkerPort` instead of extending the former
process-shaped `TranscodeExecutor` across a network boundary.
