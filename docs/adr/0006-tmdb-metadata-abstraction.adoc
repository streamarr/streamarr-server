...
Define a `MetadataProvider<T>` interface that abstracts metadata search and enrichment. TMDB is the first implementation. A factory (`MovieMetadataProviderFactory`) selects the provider based on the library's configured agent strategy.
...
* The factory pattern allows per-library provider selection (e.g., one library uses TMDB, another uses a custom source)
