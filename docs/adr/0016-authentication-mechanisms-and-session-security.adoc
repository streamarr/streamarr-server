...
The endpoint is permitAll and cookie-blind like the pre-auth endpoints (a stale `Path=/` access cookie must not 401 a key fetch); `Cache-Control: max-age` paces polling, and refetch-on-unknown-`kid` remains the rotation mechanism.
