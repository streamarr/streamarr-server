...
class KeysetConnectionsTest {
  ...
  void shouldLimitEdgesAndReportNextPageWhenLookaheadRowIsFetched() {
    var connection =
        KeysetConnections.page(List.of("a", "b", "c"), value -> value, options(null, 2));
  ...
  void shouldReturnEmptyConnectionWhenValidCursorFollowsFinalRow() {
    var connection =
        KeysetConnections.page(List.<String>of(), value -> value, options("final-cursor", 5));
  ...
  private static PaginationOptions options(String cursor, int limit) {
