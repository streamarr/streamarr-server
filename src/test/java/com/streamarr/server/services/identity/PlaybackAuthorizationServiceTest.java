...
class PlaybackAuthorizationServiceTest {
  ...
  private final AuthenticatedIdentity identity =
      AuthenticatedIdentityFixture.profileScopedBuilder().build();
  private final FakeAuthorizationService authorization = new FakeAuthorizationService(identity);
  ...
  void shouldAllowPlaybackWhenAuthorityMatchesAndCedarAllows() {
    ...
    assertThat(service.allows(identity, authority)).isTrue();
  ...
  void shouldDenyPlaybackWhenStreamAuthorityIsNotThisRequestsIdentity() {
    var own = identity.playbackAuthority();
    var other =
    ...
    assertThat(service.allows(identity, other)).isFalse();
  ...
  void shouldDenyPlaybackWhenRequestIdentityHasNoSelectedProfile() {
    var accountIdentity = AuthenticatedIdentityFixture.accountScopedBuilder().build();
    var accountAuthorization = new FakeAuthorizationService(accountIdentity);
    var accountScoped = new PlaybackAuthorizationService(accountAuthorization);
    var authority = identity.playbackAuthority();

    assertThat(accountScoped.allows(accountIdentity, authority)).isFalse();
  ...
  void shouldDenyPlaybackWhenCedarDeniesOrCannotDecide() {
    ...
    assertThat(service.allows(identity, authority)).isFalse();
