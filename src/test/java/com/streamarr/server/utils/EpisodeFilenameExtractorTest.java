...
public class EpisodeFilenameExtractorTest {
    ...
    public class SuccessfulSeasonEpisodeTests {
        ...
        Stream<DynamicNode> tests() {
            return Stream.of(
                new TestCase("when given path and S-E format", "Series/4-12 - The Woman.mp4", 4, 12),
                new TestCase("when given path and SxE format", "Series/4x12 - The Woman", 4, 12),
                new TestCase("when given S-E format", "1-12 episode title", 1, 12),
                new TestCase("when given path and S01E01 format", "/server/Temp/S01E02 foo", 1, 2)
            ).map(testCase -> DynamicTest.dynamicTest(
                testCase.name(),
                () -> {
                    var result = episodeFilenameExtractor.extract(testCase.filename()).orElseThrow();
