...
public sealed interface MetadataFetchOutcome<T> {
  ...
  default <R> MetadataFetchOutcome<R> map(Function<? super T, ? extends R> mapper) {
    return flatMap(result -> new Found<>(mapper.apply(result)));
  }

  default <R> MetadataFetchOutcome<R> flatMap(Function<? super T, MetadataFetchOutcome<R>> mapper) {
    return switch (this) {
      case Found<T>(var result) -> mapper.apply(result);
