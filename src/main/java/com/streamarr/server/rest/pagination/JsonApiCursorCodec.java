...
public class JsonApiCursorCodec {
  ...
  public void validateCursorFilter(MediaPaginationOptions decoded, MediaFilter currentFilter) {
    ...
    validateField("libraryId", cursorFilter.getLibraryId(), currentFilter.getLibraryId());
    validateField("sortBy", cursorFilter.getSortBy(), currentFilter.getSortBy());
    validateField(
        "sortDirection", cursorFilter.getSortDirection(), currentFilter.getSortDirection());
  }

  private <T> void validateField(String fieldName, T cursorValue, T currentValue) {
