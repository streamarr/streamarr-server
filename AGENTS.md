...
- Evolve existing APIs in place and update development clients to the new contract. Do not add versioned mutations or legacy adapters solely to preserve pre-production contracts.
...
- Administrative overrides are distinct mutations, service methods, typed intents, and Cedar actions;
  never widen the ordinary path with ServerAdmin authority. Name an explicit override consistently
  with the `administrativelyVerbNoun` form end to end. The prefix distinguishes an override from
  its ordinary or self-service counterpart; do not add it merely because an operation requires
  administrative authority. Resolver classes remain organized by domain.
...
This project is indexed by GitNexus as **streamarr-server** (17121 symbols, 49685 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/streamarr-server/context` | Codebase overview, check index freshness |
| `gitnexus://repo/streamarr-server/clusters` | All functional areas |
| `gitnexus://repo/streamarr-server/processes` | All execution flows |
| `gitnexus://repo/streamarr-server/process/{name}` | Step-by-step execution trace |
